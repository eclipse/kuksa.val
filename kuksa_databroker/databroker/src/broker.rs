...
use crate::query;
pub use crate::types::{ChangeType, DataType, EntryType, Value};
...
pub struct Entry {
    ...
    pub actuator_target: Option<Value>,
...
pub struct EntryUpdate {
    ...
    pub actuator_target: Option<Option<Value>>, // only for actuators
...
impl Entry {
    ...
    pub fn validate(&self, update: &EntryUpdate) -> Result<(), UpdateError> {
        ...
        if let Some(actuator_target) = &update.actuator_target {
            if let Some(value) = actuator_target {
    ...
    fn validate_value(&self, value: &Value) -> Result<(), UpdateError> {
        // Validate value
        match self.metadata.data_type {
            ...
            DataType::Int8 => match value {
                Value::Int32(value) => match i8::try_from(*value) {
                    Ok(_) => Ok(()),
            ...
            DataType::Int16 => match value {
                Value::Int32(value) => match i16::try_from(*value) {
                    Ok(_) => Ok(()),
                    Err(_) => Err(UpdateError::OutOfBounds),
                },
                _ => Err(UpdateError::WrongType),
            ...
            DataType::Uint8 => match value {
                Value::Uint32(value) => match u8::try_from(*value) {
                    Ok(_) => Ok(()),
                    Err(_) => Err(UpdateError::OutOfBounds),
                },
                _ => Err(UpdateError::WrongType),
            ...
            DataType::Uint16 => match value {
                Value::Uint32(value) => match u16::try_from(*value) {
                    Ok(_) => Ok(()),
                    Err(_) => Err(UpdateError::OutOfBounds),
                },
                _ => Err(UpdateError::WrongType),
            ...
            DataType::BoolArray => match value {
                Value::BoolArray(_) => Ok(()),
                _ => Err(UpdateError::WrongType),
            },
            DataType::StringArray => match value {
                Value::StringArray(_) => Ok(()),
                _ => Err(UpdateError::WrongType),
            },
            ...
            DataType::Int32Array => match value {
                Value::Int32Array(_) => Ok(()),
                _ => Err(UpdateError::WrongType),
            },
            DataType::Int64Array => match value {
                Value::Int64Array(_) => Ok(()),
                _ => Err(UpdateError::WrongType),
            },
            ...
            for name in self.query.input_spec.iter() {
                match all.get_by_path(name) {
                    ...
                    None => {
                        ...
                        input.add(name.to_owned(), Value::NotAvailable)
                    }
                }
            }
            Some(input)
...
async fn test_get_set_datapoint() {
    ...
    match datapoints.get_datapoint(id1).await {
        Some(datapoint) => {
            assert_eq!(datapoint.value, Value::NotAvailable);
        }
        None => {
    ...
    match datapoints.get_datapoint(id1).await {
        Some(datapoint) => {
            assert_eq!(datapoint.value, Value::Int32(100));
        }
        None => {
            panic!("data point expected to exist");
        }
...
async fn test_subscribe_and_get() {
    ...
    match stream.next().await {
        Some(query_resp) => {
            ...
            assert_eq!(query_resp.fields[0].value, Value::NotAvailable);
        }
        None => {
    ...
    match stream.next().await {
        Some(query_resp) => {
            assert_eq!(query_resp.fields.len(), 1);
            assert_eq!(query_resp.fields[0].name, "test.datapoint1");
            assert_eq!(query_resp.fields[0].value, Value::Int32(101));
        }
        None => {
            panic!("did not expect stream end")
        }
    ...
    match datapoints.get_datapoint(id1).await {
        Some(datapoint) => {
            assert_eq!(datapoint.value, Value::Int32(101));
        }
        None => {
            panic!("data point expected to exist");
        }
...
async fn test_multi_subscribe() {
    ...
    for i in 0..100 {
        ...
        if let Some(query_resp) = subscription1.next().await {
            assert_eq!(query_resp.fields.len(), 1);
            assert_eq!(query_resp.fields[0].name, "test.datapoint1");
            if let Value::Int32(value) = query_resp.fields[0].value {
                assert_eq!(value, i);
            } else {
...
async fn test_subscribe_after_new_registration() {
    ...
    match subscription.next().await {
        Some(query_resp) => {
            assert_eq!(query_resp.fields.len(), 1);
            assert_eq!(query_resp.fields[0].name, "test.datapoint1");
            assert_eq!(query_resp.fields[0].value, Value::Int32(200));
        }
        None => {
            panic!("did not expect stream end")
        }
    ...
    match datapoints.get_datapoint(id1).await {
        Some(datapoint) => {
            assert_eq!(datapoint.value, Value::Int32(200));
        }
        None => {
            panic!("data point expected to exist");
        }
    ...
    match subscription.next().await {
        Some(query_resp) => {
            assert_eq!(query_resp.fields.len(), 1);
            assert_eq!(query_resp.fields[0].name, "test.datapoint1");
            assert_eq!(query_resp.fields[0].value, Value::Int32(102));
        }
        None => {
            panic!("did not expect stream end")
        }
    ...
    match datapoints.get_datapoint(id1).await {
        Some(datapoint) => {
            assert_eq!(datapoint.value, Value::Int32(102));
        }
        None => {
            panic!("data point expected to exist");
        }
...
async fn test_subscribe_set_multiple() {
    ...
    match subscription.next().await {
        Some(query_resp) => {
            ...
            assert_eq!(query_resp.fields[1].value, Value::NotAvailable);
        }
        None => {
            panic!("did not expect stream end")
        }
    ...
    for i in 0..10 {
        if let Some(query_resp) = subscription.next().await {
            ...
            assert_eq!(query_resp.fields[1].name, "test.datapoint2");
            if let Value::Int32(value) = query_resp.fields[1].value {
                assert_eq!(value, i);
            } else {
...
async fn test_bool_array() {
    ...
    match broker
        .update_entries([(
            ...
            EntryUpdate {
                ...
                datapoint: Some(Datapoint {
                    ts,
                    value: Value::BoolArray(vec![true, true, false, true]),
                }),
                actuator_target: None,
                entry_type: None,
                data_type: None,
    ...
    match broker.get_datapoint(id).await {
        Some(datapoint) => {
            ...
            assert_eq!(
                datapoint.value,
                Value::BoolArray(vec![true, true, false, true])
            );
        }
...
async fn test_int8_array() {
    ...
    match broker
        .update_entries([(
            ...
            EntryUpdate {
                ...
                datapoint: Some(Datapoint {
                    ts,
                    value: Value::Int32Array(vec![10, 20, 30, 40]),
                }),
                actuator_target: None,
                entry_type: None,
                data_type: None,
    ...
    if broker
        .update_entries([(
            ...
            EntryUpdate {
                ...
                datapoint: Some(Datapoint {
                    ts,
                    value: Value::Int32Array(vec![100, 200, 300, 400]),
                }),
                actuator_target: None,
                entry_type: None,
                data_type: None,
    ...
    match broker.get_datapoint(id).await {
        Some(datapoint) => {
            assert_eq!(datapoint.ts, ts);
            assert_eq!(datapoint.value, Value::Int32Array(vec![10, 20, 30, 40]));
        }
        None => panic!("Expected get_datapoint to return a datapoint, instead got: None"),
    }
}
...
async fn test_uint8_array() {
    ...
    match broker
        .update_entries([(
            ...
            EntryUpdate {
                ...
                datapoint: Some(Datapoint {
                    ts,
                    value: Value::Uint32Array(vec![10, 20, 30, 40]),
                }),
                actuator_target: None,
                entry_type: None,
                data_type: None,
    ...
    if broker
        .update_entries([(
            ...
            EntryUpdate {
                ...
                datapoint: Some(Datapoint {
                    ts,
                    value: Value::Uint32Array(vec![100, 200, 300, 400]),
                }),
                actuator_target: None,
                entry_type: None,
                data_type: None,
    ...
    match broker.get_datapoint(id).await {
        Some(datapoint) => {
            assert_eq!(datapoint.ts, ts);
            assert_eq!(datapoint.value, Value::Uint32Array(vec![10, 20, 30, 40]));
        }
        None => panic!("Expected get_datapoint to return a datapoint, instead got: None"),
    }
}
...
async fn test_float_array() {
    ...
    match broker
        .update_entries([(
            ...
            EntryUpdate {
                ...
                datapoint: Some(Datapoint {
                    ts,
                    value: Value::FloatArray(vec![10.0, 20.0, 30.0, 40.0]),
                }),
                actuator_target: None,
                entry_type: None,
                data_type: None,
    ...
    match broker.get_datapoint(id).await {
        Some(datapoint) => {
            ...
            assert_eq!(
                datapoint.value,
                Value::FloatArray(vec![10.0, 20.0, 30.0, 40.0])
            );
        }
        None => panic!("Expected get_datapoint to return a datapoint, instead got: None"),
    }
