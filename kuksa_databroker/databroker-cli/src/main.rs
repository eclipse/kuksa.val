        ...
        if self.paths.children.len() > 0 {
            ...
            loop {
                match parts.next() {
                    ...
                    None => {
                        for (_, path_spec) in &path.children {
                            if path_spec.children.len() > 0 {
                                // This is a branch
                                res.push(Completion {
                                    completion: format!("{}.", path_spec.full_path),
...
fn test_entry_path_completion() {
    ...
    match completer.complete_entry_path("vehicle.") {
        Some(completions) => {
            ...
            assert_eq!(completions[0].display(), "Test.");
            assert_eq!(completions[1].display(), "AnotherTest.");
        }
        None => panic!("expected completions, got None"),
    }
