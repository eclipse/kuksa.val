...
* [API scope implementation in KUKSA.VAL](#api-scope-implementation-in-kuksaval)
  * [Format](#format)
    * [Actions](#actions)
    * [Paths](#paths)
  * [Example 1](#example-1)
  * [Example 2](#example-2)
  * [Example 3](#example-3)
  * [Scope (possible extensions)](#scope-possible-extensions)
...
## Introduction
Authorization in kuksa.val is designed to be compatible with the OAuth2 specification, which
establishes a framework for handing out (issuing) delegated access to restricted resources.

This sort of authorization framework is well suited for an environment where
third party applications need delegated access to a resource, while at the
same time restricting the scope of this access to minimum.
...
Access tokens are supplied by adding an authorization header to the HTTP / gRPC requests.

`Authorization: Bearer TOKEN`

The TOKEN is a JWT access token as specified in the [RFC](https://datatracker.ietf.org/doc/html/rfc9068).
...
from the authorization server. The result is that the client (e.g. a third party app) using
the issued access token only gets a subset of the resource owners access rights.
...
There are currently no plans in KUKSA.VAL to verify that the subject itself has the required
access rights, but there is nothing in the design to prevent this from future inclusion.
What this means is that any subject authorized to get a valid access token is assumed to have
these (full) rights, which are only limited by the scope provided.
...
## API scope implementation in KUKSA.VAL

Access to VSS entries / signals are limited by the scope provided in the JWT access token.

Principally, the access rights in KUKSA.VAL govern which VSS entries / signals the client is
allowed to read or modify. This is represented in the scopes by utilizing the concept of
"prefix scopes" that consist of a VSS path, prefixed with an action. This provides a more
flexible way to specify the scope than what would be possible if using predefined scopes
that mapped to certain use cases.
